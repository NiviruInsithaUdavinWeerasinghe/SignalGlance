    ...
    public partial class InstallerWindow : Window
    {
        ...
        private async Task RunInstallationAsync()
        {
            try
            {
                ...
                // Terminate any existing running instances of the app to prevent file-locking
                try
                {
                    var runningProcesses = System.Diagnostics.Process.GetProcessesByName("SignalGlance");
                ...
                for (int i = 0; i < filesToCopy.Length; i++)
                {
                    ...
                    {
                        // Fallback if running directly from dotnet run or folder where only exe/dll exist
                        if (file.EndsWith(".exe") || file.EndsWith(".dll"))
                        {
                            throw new FileNotFoundException($"Setup files are missing! Could not locate: {srcPath}");
                        }
                        continue; // Skip optional files like deps/config if they aren't generated/needed in source
                    }
